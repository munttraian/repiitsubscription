...
if (!function_exists('getProductAttribute')) {
    ...
    function createAttribute($code, $label, $attribute_type, $product_type = 0, $is_filterable = 0, $attribute_exists = 0)
    {
        ...
        try {
            ...
            echo '<p>Sorry, error occured while trying to save the attribute ' . $code . '. Error: ' . $e->getMessage() . '</p>' . "\n";
        }
    }
}
//========================Start Installer==================//
...
//======create attributes
$arrAttributes = array(
    0 => array("code" => "clickdownload_price",
        "label" => "Click/Download Price",
        "attribute_type" => "price",
        "product_type" => 0,
        "is_filterable" => 0,
        "attribute_exists" => 0),
    1 => array("code" => "clickdownload_enable",
        "label" => "Click/Download Active",
...
//get group name
$clickdownloadGroupName = Mage::helper('Repiit_Click')->getClickDownloadAttributeGroup();
...
foreach ($collection as $item)
{
    ...
    $attributeGroup = getAttributeGroup($attributeSetId, $clickdownloadGroupName);

    //create group if not exists
    if (!$attributeGroup || !$attributeGroup->getId())
    {
        createAttributeGroup($attributeSetId, $clickdownloadGroupName);
        $attributeGroup = getAttributeGroup($attributeSetId, $clickdownloadGroupName);
    }
